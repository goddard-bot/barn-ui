...
def calc_friend(hap, hun, dis, sic):
    ...
    friend_val = int(friend)
    return friend_val


def feed_robot(hap, hun):
    food_val = randint(1, 20)
    hun += food_val
    hap += (food_val / 2)
    print("You fed the robot for", food_val, "points!")
...
def heal_robot(hap, dis, sic):
    med_val = randint(1, 30)
    sic -= med_val
    hap += (med_val / 4)
    dis -= (med_val / 4)
    print("You healed the robot for", med_val, "points!")
...
def main():
    ...
    while True:
        ...
        dif = (stop - start) / 2
        happiness -= int(dif)
        sick_chance = randint(1, 100)
        if sick_chance > 95:
            sickness += 5
        if sickness != 0:
            sickness += 10
        happiness, hunger, discipline, sickness = cap_emots(happiness,
                                                            hunger,
                                                            discipline,
                                                            sickness)
